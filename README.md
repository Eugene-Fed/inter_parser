...
- [ ] Добавить асинхронности/многопоточности коду, чтобы распараллелить отправку запросов. Вероятно лучше использовать `concurrent.futures` вместо `asyncio` (или вместе с ним, если не получится "запутать" сервер и придется ожидать ответа по каждому отдельному запросу).
